...
[![Build Status](https://secure.travis-ci.org/orata/OrataPayboxBundle.png)](http://travis-ci.org/orata/OrataPayboxBundle)
[![Latest Stable Version](https://poser.pugx.org/orata/paybox-bundle/v/stable.svg)](https://packagist.org/packages/orata/paybox-bundle)
...
OrataPayboxBundle makes the use of [Paybox](http://www.paybox.com) payment system easier by doing all the boring things for you.

OrataPayboxBundle silently does :
...
```bash
composer require orata/paybox-bundle
```
...
public function registerBundles()
{
    return array(
        // ...
        new Orata\Bundle\PayboxBundle\OrataPayboxBundle(),
        // ...
    );
}
```
...
# Orata Paybox Bundle
orata_paybox:
    resource: '@OrataPayboxBundle/Resources/config/routing.yml'
```
...
public function callAction()
{
    ...
    $paybox->setParameters(array(
        ...
        'PBX_TYPECARTE'    => 'CB',
        'PBX_EFFECTUE'     => $this->generateUrl('orata_paybox_sample_return', array('status' => 'success'), UrlGeneratorInterface::ABSOLUTE_URL),
        'PBX_REFUSE'       => $this->generateUrl('orata_paybox_sample_return', array('status' => 'denied'), UrlGeneratorInterface::ABSOLUTE_URL),
        'PBX_ANNULE'       => $this->generateUrl('orata_paybox_sample_return', array('status' => 'canceled'), UrlGeneratorInterface::ABSOLUTE_URL),
        'PBX_RUF1'         => 'POST',
        'PBX_REPONDRE_A'   => $this->generateUrl('orata_paybox_ipn', array('time' => time()), UrlGeneratorInterface::ABSOLUTE_URL),
    ));
...
{
    return $this->render(
        'OrataPayboxBundle:Sample:return.html.twig',
        array(
            'status'     => $status,
